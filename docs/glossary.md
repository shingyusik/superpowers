...
워크플로우 마지막에 항상 호출되는 하네스 개선 전담 에이전트.

Improvement Agent는 작업을 직접 구현하거나 최종 결과를 승인하지 않는다. 대신 전체 작업 기록을 검토해 하네스 구조를 어떻게 개선할지 판단한다.
...
Improvement Agent가 작업 기록을 검토해 발견한 실패, 반복 패턴, 좋은 운영 방식을 하네스 구조에 반영하는 과정.
