...
Harness Improvement Agent는 작업을 직접 구현하거나 최종 결과를 승인하지 않는다. 대신 전체 작업 기록을 검토해 하네스 구조를 어떻게 개선할지 판단한다.
...
Harness Improvement Agent가 작업 기록을 검토해 실패, 반복 패턴, 좋은 운영 방식을 찾고 하네스 개선 후보를 남기는 과정.

MVP에서는 Improve와 Evolve를 분리하지 않는다. 개선 제안과 반영 판단을 하나의 단계에서 다룬다.
