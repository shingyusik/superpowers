...
- **Harness Improvement Agent**
  - 모든 워크플로우 마지막에 호출되는 상위 하네스 유지보수 에이전트다. 전체 작업 기록을 검토하고 스킬, 에이전트, 산출물 템플릿, task contract 템플릿, workflow 문서를 작고 검증 가능한 단위로 직접 개선한다.
...
6. **Improve**
   - Harness Improvement Agent를 호출해 전체 작업 기록을 검토하고, 필요한 경우 하네스 문서, 스킬, 에이전트, 산출물 템플릿, task contract 템플릿을 직접 개선한다.
...
- 하네스 개선은 메인 에이전트가 겸임하지 않고, 가장 많은 작업 기록을 읽는 전용 상위 개선 에이전트가 맡는다.
...
- **Compound skills**: `ce-compound`, `ce-compound-refresh`, `ce-sessions`
- **Compound agents**: Claude-compatible prompts under `agents/*.md`
- **Codex agents**: Codex-compatible TOML agents under `agents-codex/compound-engineering/*.toml`
- **Caveman**: `caveman` skill only
